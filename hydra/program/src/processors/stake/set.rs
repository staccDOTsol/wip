...
use basic_oracle::PROGRAM_SEED;
use basic_oracle::MyProgramState;
...
pub fn set_token_member_stake(ctx: Context<SetTokenMemberStake>, mut shares: u64) -> Result<()> {
    ...
    let program = &mut ctx.accounts.program.load()?;
    let bsol_price = program.bsol_price;
    let og_shares = shares.clone();
    shares = shares.checked_mul(bsol_price as u64).or_arith_error()?;
    ...
    anchor_spl::token::transfer(cpi_ctx, og_shares)?;
    Ok(())
}
