...
impl AnchorDeserialize for U192 {
    fn deserialize_reader<R: std::io::Read>(reader: &mut R) -> Result<U192, std::io::Error> {
        let mut buf = [0u8; 24];
        reader.read(&mut buf)?;
...
impl AnchorSerialize for U192 {
    ...
    where
        W: std::io::Write,
    {
        writer.write(&self.to_owned().as_u128().to_le_bytes())?;
        Ok(())
    }
}
...
#[derive(Copy, Default, PartialEq, PartialOrd, Eq, Ord)]
